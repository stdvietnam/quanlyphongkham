...
Public Class frmTKTonKhoKhoaDuoc
    ...
    Private Sub Load_CnnInfor()
        ...
        dt = BusinessLogic.Data.GetDataset("TK_KhoDuoc_TongHop", New Generic.List(Of String) From {"@tungay", "@denngay", "@kho", "@thuoc", "@nhom", "@manhomxetnghiem"}, New Generic.List(Of Object) From {Me.dtTuNgay.DateTime, Me.dtDenNgay.DateTime, Me.cmbkho.EditValue, Me.cmbTenThuoc.EditValue, Me.cmbnhom.EditValue, cmbNhomsp.EditValue}).Tables(0)
    ...
    Private Sub LoadCombobox()
        ...
        Load_ComboBox("Select makho,tenkho from kho_hang where makho=N'KT'", cmbkho, CNNQLPHONGKHAM)
        Load_ComboBox("Select Manhom[Mã],Tennhom[Tên nhóm] from nhom_sp Where Manhom='10' order by manhom", Me.cmbnhom, CNNQLPHONGKHAM)
        Load_ComboBox("select MaNhom,TenNhom from Tb_NhomXetNghiem", cmbNhomsp, CNNQLPHONGKHAM)
        Me.cmbkho.EditValue = "KT"
        Me.cmbnhom.EditValue = "10"
    End Sub
    ...
    Private Sub cmbNhomsp_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles cmbNhomsp.EditValueChanged
        If cmbnhom.EditValue <> "" Then
            Load_ComboBox("Select Masp,Tensp from San_Pham where Manhomxetnghiem=N'" & cmbNhomsp.EditValue & "'", cmbTenThuoc, CNNQLPHONGKHAM)
        Else
            Load_ComboBox("Select Masp,Tensp from San_Pham where Loai in (0)", cmbTenThuoc, CNNQLPHONGKHAM)
        End If
    End Sub
    ...
    Sub Link_CreateMarginalHeaderArea(ByVal sender As Object, ByVal e As CreateAreaEventArgs)
        Dim Brick() As PageInfoBrick = {e.Graph.DrawPageInfo(PageInfo.None, "THỐNG KÊ TỒN KHO THUỐC", Color.DarkBlue, New RectangleF(0, 0, 500, 35), BorderSide.None), e.Graph.DrawPageInfo(PageInfo.None, "Từ ngày " & dtTuNgay.DateTime.ToString("dd/MM/yyyy") & " đến ngày " & dtDenNgay.DateTime.ToString("dd/MM/yyyy") & "", Color.DarkBlue, New RectangleF(0, 0, 100, 20), BorderSide.None)}
